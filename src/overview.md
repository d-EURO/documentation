...
The Decentralized Euro can be used to make payments in Euros. Please consult the [landing page](https://dEuro.com/) for a list of apps and services that help in using the Decentralized Euro as a means of payment, as well as a list of bridged Decentralized Euro token on other networks than Ethereum mainnet. Payments typically concern small amounts and therefore a layer two instance of the token might be preferred over the mainnet instance.
...
Anyone can mint new Decentralized Euros against a collateral using the built-in borrowing mechanism. All borrowing is based on what we refer to as _positions_. In Liquity, these are called _troves_ and in the Maker system _vaults_. They all refer to the account of a user within the system that holds a positive balance of a collateral asset and a negative balance of Decentralized Euros that must be repaid in order to get the collateral back. Interest accrues continuously on the position and is paid when the position is closed or modified. Some of the minted Decentralized Euros are also held back as a reserve in case the position has to be liquidated.

## Technical Architecture

The Decentralized Euro system consists of a set of smart contracts on Ethereum mainnet. The two token contracts serve as a foundation for everything else. The dEURO token accepts an arbitrary number of contracts that have the power to mint and burn dEURO. Anyone can propose new such contracts and once they pass the governance process, they can start minting and burning dEURO. The currently authorized minter contracts include the [V2 and V3 minting hubs](smart-contracts.md), the V2 and V3 savings modules, the [stablecoin bridges](swap.md) to other EUR stablecoins, the [V2 FrontendGateway](frontend-rewards.md) for referral rewards, and the V2 CoinLendingGateway for native ETH positions. The nDEPS token has built-in governance features and holds the equity capital of the system; the [DEPSwrapper](smart-contracts.md#depswrapper-deps) contract exposes a non-voting `DEPS` token for use in liquidity pools.
